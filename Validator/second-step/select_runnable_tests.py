...
def get_correct_tcs(gen_tests, after_rm, inject_point, curdir, file, method, da):
    ...
    if add_all_tests == 1:
        ...
        for i, test_code in enumerate(parsable_tcs):
            ...
            # out = os.system(
            #     f'cd tmp/{method}/defects4j_projects/{project_name} && rm -rf build && rm -rf build-tests && rm -rf target && JAVA_HOME=/mnt/c/Program Files/Java/jdk-1.8/jre defects4j compile')
            if out == 0:
...
def save_results_to_file(project_name, total_gen_tcs, not_parsable_tcs, not_compilable_tcs, method, da):
    # 定义文件名
    file_name = f'results_{method}_{da}_{project_name}.txt'

    # 打开文件并写入结果
    with open(file_name, 'w') as f:
        f.write(f'Project Name: {project_name}\n')
        f.write(f'Method: {method}\n')
        f.write(f'Data Augmentation: {da}\n')
        f.write(f'Total Generated Tests: {total_gen_tcs}\n')
        f.write(f'Not Parsable Tests: {not_parsable_tcs}\n')
        f.write(f'Not Compilable Tests: {not_compilable_tcs}\n')
    ...
    if method == 'gpt4' or method == 'a3test':
        ...
        for project_name, (test_path, split_length) in project_names.items():
            ...
            for da in ['sft-new']:
                ...
                print()
                # 保存结果到文件
                save_results_to_file(project_name, total_gen_tcs, not_parsable_tcs, not_compilable_tcs, method, da)
    elif method == 'bart':
        for project_name, (test_path, split_length) in project_names.items():
            for da in ['da', 'sft']:
                if os.path.exists(f'defects4j_projects/{project_name}'):
