...
if __name__ == "__main__":
    ...
    if method == 'codet5':
        for prj in ['compress', 'gson', 'jacksonCore', 'jacksonDatabind', 'jsoup']:
            for da in ['sft-new']:
                out = f"../eva/codet5/{da}/{prj}.tests"
                info = f"../eva/defects4j/{prj}_test_info_0.txt"
                prepare(out, info, method, da)
    elif method == 'bart':
        for prj in ['compress', 'gson', 'jacksonCore', 'jacksonDatabind', 'jsoup']:
            for da in ['da', 'sft']:
                out = f"../eva/bart/{da}/{prj}.tests"
                info = f"../eva/defects4j/{prj}_test_info_0.txt"
                prepare(out, info, method, da)
    elif method == 'gpt4':
        for prj in ['compress', 'gson', 'jacksonCore', 'jacksonDatabind', 'jsoup']:
            out = f"../eva/gpt4/{prj}_generated_0.tests"
            info = f'../eva/gpt4/{prj}_test_info_0.txt'
            prepare(out, info, method, '')
    elif method == 'a3test':
        for prj in ['compress', 'gson', 'jacksonCore', 'jacksonDatabind', 'jsoup']:
            out = f"../eva/a3test/{prj}.tests"
            info = f"../eva/defects4j/{prj}_test_info_0.txt"
            prepare(out, info, method, '')
    else:
