...
class StructFTEncoderAttention(T5Attention):
...
class StructFTEncoderLayerSelfAttention(T5LayerSelfAttention):
    def __init__(self, config, dtype):
        super().__init__(config, True)
        self.SelfAttention = StructFTEncoderAttention(config, dtype)
...
class StructFTEncoderBlock(T5Block):
    def __init__(self, config, dtype):
        super().__init__(config, True)
        self.layer = nn.ModuleList()
        self.layer.append(StructFTEncoderLayerSelfAttention(config, dtype))
...
class StructFTEncoderStack(T5Stack):
    def __init__(self, config):
        super().__init__(config)
        self.block = nn.ModuleList([StructFTEncoderBlock(config, self.dtype) for i in range(config.num_layers)])
...
@dataclass
class StructFTOutput(ModelOutput):
...
class StructFTForConditionalGeneration(T5ForConditionalGeneration):
    def __init__(self, args):
        ...
        # encoder
        self.encoder = StructFTEncoderStack(config)
        ...
        if args.model_size!='small':
            args.logger.write('\nLoading StructFT weights from CodeT5.')
            pt_model = T5ForConditionalGeneration.from_pretrained('codet5-base')
