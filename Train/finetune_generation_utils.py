...
from modeling_structft import StructFTForConditionalGeneration
import torch
...
def parse_args():
    ...
    # parser.add_argument("--load_model_path", default='saved_models/compress_generation/lr_1e-05/checkpoint_best_at_12536.bin', type=str)
    ...
    # max lengths
    parser.add_argument('--max_length_source_text', type=int, default=500)
    parser.add_argument("--max_ast_depth", default=17, type=int)
    parser.add_argument('--max_length_target_code', type=int, default=500) # for get_batch(), max_length_target_code<=max_length_source_code
    ...
    parser.add_argument('--validate_every', type=int, default=1) # validate every __ training batches
    parser.add_argument('--eval_batch_size', type=int, default=4)
    parser.add_argument('--num_eval_batches_aux', type=int, default=5000)
    parser.add_argument('--num_eval_batches_bleu', type=int, default=5000)
    ...
    parser.add_argument('--print_train_loss_every', type=int, default=1) # print train loss every __ training batches
    parser.add_argument('--checkpoint_every', type=int, default=1) # save best model weights for every __ training batches  一个训练轮次
    parser.add_argument('--patience', type=int, default=9999) # no. of validation steps with no improvement after which to stop training
    parser.add_argument('--max_steps', type=int, default=1)  #根据数据集大小进行修改9500
    parser.add_argument('--seed', type=int, default=1234) # for RNGs
...
def set_output_dir(args):
    if args.output_dir is not None:
        return
    args.output_dir = 'saved_models/compress_generation/'
...
def read_data(args):
    data_by_split = pickle.load(open('data/compress_generation/preprocessed_data_by_split.pkl', 'rb'))
...
def test(model, data, args, train_step=None, split='test'):
    ...
    true_filepath = 'data/compress_generation/'+split+'_'+args.target_lang+'.txt'
    ...
    # return {'CodeBLEU': 1, 'BLEU': 1, 'xMatch': 1}
