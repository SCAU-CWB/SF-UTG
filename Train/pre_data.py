...
def get_code_tokens_ranges(data, code_col, code_tokens_col):
    ...
    for row in pbar:
        code_tokens = [tokenizer.decode(ct) for ct in getattr(row, code_tokens_col)][
                      1:-1]  # 1:-1 to remove <s> and </s>
...
def read_examples(split):
    if split in ['valid', 'validation']:
        split = 'dev'
    examples = []
    with open('data/jsoup/' + split + '.json') as f:
...
save_dir = 'data/jsoup_generation/'
